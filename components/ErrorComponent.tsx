...
export const ErrorComponent = ({ error }) => {
  ...
  const extractErrorMessage = err => {
    ...
    if (err.response?.data?.code === 'AUTH_TOKEN_EXPIRED' || err.code === 'AUTH_TOKEN_EXPIRED') {
  ...
  if (error && error.status == 500) {
    if (error.name == 'AxiosError') {
      message =
        'Connection to server failed. Please check your internet connection or call the geeks';
    }
  }
  ...
  return (
    <Card>
      <CardBody>
        <h1 data-testid="error-title" className="text-red-600">
          {message}
        </h1>
        {message !== extractErrorMessage(error) && <h4 data-testid="error-message">{message}</h4>}
        <br />
        {isAuthError && (
          <div className="flex gap-2 mt-4 mb-4">
            ...
            <Button color="default" variant="flat" onPress={() => navigate('/')}>
              Go to Home
            </Button>
          </div>
        )}
        {/*getErrorDetails()*/}
        {NODE_ENV === 'development' && getErrorDetails()}
      </CardBody>
    </Card>
  );
};
