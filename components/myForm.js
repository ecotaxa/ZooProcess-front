...
export function MyForm(props) {
  ...
  const myElement = formitem => {
    ...
    return (
      <Grid key={formitem.name} xs={formitem.xs} sm={formitem.sm} item={true}>
  ...
  const formElements = (myJsonForm = []) => {
    return myJsonForm.map(input => (
      <div className="grid" key={input.title}>
        <Grid xs={12} item marginTop={6} marginBottom={2} key="title">
          <Typography variant="subtitle1" align="center" gutterBottom>
            {input.title}
          </Typography>
        </Grid>
        <Grid container spacing={0} rowSpacing={3} columnSpacing={1} xs={12} item>
    ...
    <form onSubmit={onSubmitHandler}>
      <Card>
        <CardHeader className="flex flex-col">
          ...
          <h4 variant="subtitle1" color="textSecondary">
            {subtitle}
          </h4>
        ...
        <CardFooter>
          <div className="gridjustify-items-end">
            ...
            <div className="flex flex-row-reverse">
              <Button type="submit" variant="solid" color="primary" isDisabled={!isDataModified}>
                {isUpdating ? btn.submitting : btn.submit}
              </Button>
              <Spacer x={2} />

              <Button type="reset" variant="flat" color="primary" onClick={cancel}>
                {btn.cancel}
              </Button>
