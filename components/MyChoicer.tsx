...
export const MyChoicer: React.FC<MyChoicerProps> = ({
  backgrounds,
  onSelect,
  ...
  const [selectedBackground, setSelectedBackground] = useState<Set<string>>(
    new Set([selectedBackgroundId])
  );
        ...
        <TableBody>
          {backgrounds.map((background: ChoicerInterface) => (
            <TableRow key={background.id}>
              ...
              <TableCell>
                <Button onPress={() => setShowImage(background.name)}>
                  <EyeIcon />
                </Button>
              </TableCell>
            </TableRow>
