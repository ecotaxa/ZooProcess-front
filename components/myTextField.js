...
export function MyTextField(props) {
  ...
  const adornment = () => {
    ...
    if (props.endAdornment.pos && props.endAdornment.text) {
      if (props.endAdornment.pos === 'end')
        ...
        return {
          endAdornment: <InputAdornment position="end">{props.endAdornment.text}</InputAdornment>,
        };
      else if (props.endAdornment.pos === 'start')
        return {
          startAdornment: (
            <InputAdornment position="start">{props.endAdornment.text}</InputAdornment>
          ),
        };
      else console.warning('Unknow endAdornment.pos value');
    }
    return {};
  };
  ...
  const MUI_Text = () => {
    <TextField
      ...
      error={hasError}
      helperText={
        hasError ? (props.helperText ? props.helperText : 'Error - not an acceptable value') : ''
      }
    />;
  };
