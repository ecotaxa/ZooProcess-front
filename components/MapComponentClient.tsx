...
import { Input, Card, CardBody, Select, SelectItem, Button, CardFooter } from '@heroui/react';
...
const MapComponentClient: React.FC<MapComponentProps> = ({
  ...
  useEffect(() => {
    ...
    const shouldBePolar =
      (startLat !== undefined && (Math.abs(startLat) > 75 || Math.abs(startLat) < -75)) ||
      (endLat !== undefined && (Math.abs(endLat) > 75 || Math.abs(endLat) < -75));
  ...
  useEffect(() => {
    ...
    if (startLat !== undefined && startLng !== undefined) {
      onCoordsChange(
        [startLat, startLng],
        endLat !== undefined && endLng !== undefined ? [endLat, endLng] : undefined
      );
    }
  ...
  const convertToDMS = (
    decimal: number,
    isLongitude: boolean
  ): { deg: number; min: number; sec: number; dir: string } => {
  ...
  const updateDMSState = (
    ...
    setter: React.Dispatch<
      React.SetStateAction<{ deg: number; min: number; sec: number; dir: string }>
    >,
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          >
            <Input
              label={t('Start_Latitude')}
              value={startLat ? startLat.toString() : ''}
              onChange={e =>
                e.target.value &&
                updateDecimalState(
                  e.target.value,
                  setStartLat as React.Dispatch<React.SetStateAction<number>>,
                  setStartLatDMS,
                  false
                )
              }
            />
            <Input
              label={t('Start_Longitude')}
              value={startLat ? startLng?.toString() : ''}
              onChange={e => {
                e.target.value &&
                  updateDecimalState(
                    e.target.value,
                    setStartLng as React.Dispatch<React.SetStateAction<number>>,
                    setStartLngDMS,
                    true
                  );
              }}
        ...
        <div style={{ display: 'flex', alignItems: 'center' }}>
          ...
          >
            <Input
              label={t('End_Latitude')}
              type="text"
              value={endLat !== undefined ? endLat.toString() : ''}
              onChange={e =>
                updateDecimalState(
                  e.target.value,
                  setEndLat as React.Dispatch<React.SetStateAction<number>>,
                  setEndLatDMS,
                  false
                )
              }
            />
            <Input
              label={t('End_Longitude')}
              type="text"
              value={endLng !== undefined ? endLng.toString() : ''}
              onChange={e =>
                updateDecimalState(
                  e.target.value,
                  setEndLng as React.Dispatch<React.SetStateAction<number>>,
                  setEndLngDMS,
                  true
                )
              }
        ...
        <CardFooter>
          ...
          >
            ...
            >
              ...
              <SelectItem key="dms" value="dms">
                {t('DMS')}
              </SelectItem>
            </Select>

            {coordinateFormat === 'decimal' ? DecimalFormatButton() : DMSButton()}
            <Button onPress={clearEndPoint} style={{ marginTop: '20px' }}>
              {t('Clear_End_Point')}
            </Button>
          </div>
        </CardFooter>
