...
const ScannerComponent = (props: any) => {
  ...
  useEffect(() => {
    ...
    const instrumentSettings = instruments.find(
      instrument => instrument.id === scannerValues.id
    )?.ZooscanCalibration;
    console.debug('instrumentSettings: ', instrumentSettings);
    if (instrumentSettings && instrumentSettings.length > 0) {
      const settingsWithNoArchive = instrumentSettings.filter(
        setting => setting.archived === false
      );
  ...
  useEffect(() => {
    ...
    if (props.value) {
      ...
      const instrumentSettings = instruments.find(
        instrument => instrument.id === props.value.id
      )?.ZooscanCalibration;
      if (instrumentSettings && instrumentSettings.length > 0) {
        const settingsWithNoArchive = instrumentSettings.filter(
          setting => setting.archived === false
