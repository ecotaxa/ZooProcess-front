...
const ViewPage: FC<pageProps> = ({ sample, subsample }) => {
  ...
  useEffect(() => {
    const groupedByType = subsample.scan.reduce(
      (acc, scan) => {
        if (!acc[scan.type]) {
          acc[scan.type] = [];
        }
  ...
  const showScan = () => {
    if (!hasScanData) {
      return <p>No scan data available for this subsample.</p>;
    }
    ...
    if (!background) {
      return (
        <div>
          ...
          <Link href={backgroundURL} className="bg-primary text-white px-4 py-2 rounded-lg">
            Choose background
          </Link>
        </div>
      );
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <h1>View Page</h1>
          <div>{subsample.scan.length} images associated</div>
          {showScanList()}
        </CardHeader>
        <CardBody>
        ...
        <CardFooter>
          <Button as={Link} href="./vignettes" className="bg-primary text">
            Go to Vignettes
          </Button>
        </CardFooter>
      </Card>
