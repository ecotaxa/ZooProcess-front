...
export function ProcessTimeline(param: {
  ...
  const onCancel = () => {
    navigate(`/projects/${project.id}/samples/${sample.id}`);
  };
    ...
    switch (current) {
      ...
      case eState.background: {
        ...
        const formatData = (data: any) => {
          ...
          const backgrounds = Object.keys(data)
            .map(_scan => {
          ...
          const filtered = backgrounds.filter(
            (background: any) => background.type == 'MEDIUM_BACKGROUND'
          );
          console.log('filtered backgrounds data: ', filtered);

          let sorted = filtered.sort(
            (a: any, b: any) => new Date(b.utc).getTime() - new Date(a.utc).getTime()
          );
          ...
          if (sorted && sorted.length > 0) {
            ...
            key = (sorted as any).reduce((prev: any, current: any) => {
              return Math.abs(new Date(current.date).getTime() - subsampleDate.getTime()) <
                Math.abs(new Date(prev.date).getTime() - subsampleDate.getTime())
                ? current
                : prev;
            }).id;
            console.log('key: ', key);
      ...
      case eState.process:
        navigate(`/projects/${project.id}/samples/${sample.id}/subsamples/${subsample.id}/process`);
        return <></>;
