...
export function CalibrationTable(props: {
  instrument: Instrument;
  calibrations?: Array<ICalibration>;
    ...
    async sort({ items, sortDescriptor }) {
      return {
        items: items.sort((a, b) => {
          ...
          let cmp =
            ((parseInt(first as string) || first) ?? 0) <
            ((parseInt(second as string) || second) ?? 0)
              ? -1
              : 1;
  ...
  const renderCell = React.useCallback(
    (calibration: CalibrationKey, columnKey: keyof CalibrationKey | 'actions') => {
            ...
            <div className="relative flex items-center gap-2" key={key(calibration.id, 'action')}>
              ...
              <Tooltip color="danger" content="Archive calibration">
                <Link
                  ...
                  onPress={() => {
                    onPressArchiveCalibration(calibration.id);
                  }}
                >
      ...
      <TableBody items={list.items}>
        {(item: CalibrationKey) => (
          <TableRow key={key(item.id, 'tr')}>
            {columnKey => (
              <TableCell key={key(item.id, columnKey as string)}>
                {renderCell(item, columnKey as keyof CalibrationKey | 'actions')}
              </TableCell>
            )}
          </TableRow>
        )}
